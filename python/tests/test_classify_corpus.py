...
def test_classify_message_json_in_code_fence_parsed():
    """Models that wrap JSON in ```json...``` fences should still parse correctly."""
    client = make_mock_client(
...
def test_classify_message_json_in_plain_fence_parsed():
    client = make_mock_client(
...
# ---------------------------------------------------------------------------
# _parse_label_response — code fence stripping
# ---------------------------------------------------------------------------


def test_parse_label_response_plain_json():
    result = _parse_label_response('{"label": "delete", "confidence": 0.9, "reason": "x"}')
    assert result["label"] == "delete"


def test_parse_label_response_json_fence():
    raw = '```json\n{"label": "keep", "confidence": 0.8, "reason": "y"}\n```'
    result = _parse_label_response(raw)
    assert result["label"] == "keep"


def test_parse_label_response_plain_fence():
    raw = '```\n{"label": "uncertain", "confidence": 0.5, "reason": "z"}\n```'
    result = _parse_label_response(raw)
    assert result["label"] == "uncertain"


def test_parse_label_response_fence_no_trailing_newline():
